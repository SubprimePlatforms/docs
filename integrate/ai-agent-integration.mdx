...
DeepIDV provides a hosted MCP server at `https://mcp.deepidv.com`.
...
<Steps>
  ...
  <Step title="Add a custom remote MCP server">
    Create a new custom MCP connector and use `https://mcp.deepidv.com` as the server URL.

    <Warning>
      Do not append `/mcp` or any other path.
    </Warning>
  </Step>
