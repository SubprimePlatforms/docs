...
Returns a `FAILED` session — a session that was auto-failed after reaching the workflow's configured number of failed attempts. `session_progress` is `COMPLETED` and `meta_data.failure_data` is populated with the recorded attempts:
...
See [`failure_data`](/api-reference/sessions/retrieve-session#failure-data-object) for the full field reference.
