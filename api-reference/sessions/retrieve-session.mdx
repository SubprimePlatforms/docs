...
| Field | Type | Description |
|-------|------|-------------|
| `reason` | string | The check that failed. One of `NO_FACE_DETECTED`, `OCR_NAME_MISSING`, `DOC_TYPE_NOT_ALLOWED`, `ID_TYPE_UNRECOGNIZED`, or `ID_TYPE_LOW_CONFIDENCE`. The two `ID_TYPE_*` values are emitted only when the workflow's ID confidence threshold is enabled (driver's licence / passport): `ID_TYPE_UNRECOGNIZED` when the document type could not be read, and `ID_TYPE_LOW_CONFIDENCE` when it was read below the configured confidence. New values may be added over time — consumers should treat this as an open string. |
