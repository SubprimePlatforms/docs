...
| `failure_data` | object | Auto-failure attempt history (only present once at least one failed attempt has been recorded — see below) |
...
| Field | Type | Description |
|-------|------|-------------|
| `reason` | string | The check that failed. Currently one of `NO_FACE_DETECTED`, `OCR_NAME_MISSING`, or `DOC_TYPE_NOT_ALLOWED`. New values may be added over time — consumers should treat this as an open string. |
