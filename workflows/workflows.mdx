...
<Note>
  The four iGaming steps (`IP_JURISDICTION`, `VPN_DETECTION`, `INJECTION_DETECTION`, `ANTI_CHEAT`) are anti-fraud checks for gaming onboarding. See the [iGaming guide](/igaming/overview) for how they work and [Create workflow → Step configuration](/api-reference/workflows/create-workflow#step-configuration) for every config field and default.
</Note>
