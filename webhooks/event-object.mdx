...
See [`failure_data`](/api-reference/sessions/retrieve-session#failure-data-object) for the full field reference. `reason` is intentionally a loose string — new values may be added over time, so consumers should default-handle unknown values.
...
| `session.status.failed` | Notify applicant, surface a "contact support" path, and log `meta_data.failure_data.attempts` for diagnostics |
